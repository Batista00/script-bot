...
test("owner edits business-scoped settings; real dispatch requires explicit confirmation",async()=>{
  ...
  mount();await screen.findByLabelText("Nombre del asistente");
  await userEvent.click(screen.getByLabelText("Despachar automáticamente al proveedor tras el pago"));
...
test("failed inbox messages provide a scoped recovery action",async()=>{
  ...
  mount();await userEvent.click(await screen.findByRole("button",{name:"Reprocesar mensaje"}));
