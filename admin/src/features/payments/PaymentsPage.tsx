...
export function PaymentsPage() {
  ...
  return <><PageHeader title="Pagos" description="Mercado Pago se confirma por webhook. Las transferencias solo pueden aprobarlas owner/admin tras verificar el abono." /><div className="filter-bar"><SelectField label="Estado" value={status} onChange={(event) => { setStatus(event.target.value); setOffset(0); }}><option value="">Todos</option>{Object.entries(PAYMENT_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField><Field label="Order ID" value={orderId} onChange={(event) => { setOrderId(event.target.value); setOffset(0); }} placeholder="UUID exacto" /></div>
