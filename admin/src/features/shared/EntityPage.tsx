...
function formPayload(form: HTMLFormElement, fields: FormField[]): Record<string, unknown> {
  ...
  for (const field of fields) {
    ...
    else if (raw !== "") payload[field.name] = field.kind === "number" ? Number(raw)
      : field.kind === "json" ? JSON.parse(raw) as unknown : raw;
  }
  return payload;
}
...
export function EntityPage<T extends { id: string; status?: string }>(props: EntityPageProps<T>) {
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: async () => {
      await client.invalidateQueries({ queryKey: [props.resource, business.id] });
      toast("Registro eliminado.");
    },
  });
  function remove(record: T) {
    if (!window.confirm("¿Eliminar este registro? Si tiene historial comercial, el sistema exigirá desactivarlo.")) return;
    removeMutation.mutate(record);
  }
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const fields = props.fields.filter((field) => editing !== "new" || !field.editOnly);
    let payload: Record<string, unknown>;
    try { payload = formPayload(event.currentTarget, fields); }
    catch { setFormError("El JSON ingresado no es válido."); return; }
    setFormError(null);
    if (payload.status === "inactive" && editing !== "new" && !window.confirm("¿Confirmas la desactivación?")) return;
