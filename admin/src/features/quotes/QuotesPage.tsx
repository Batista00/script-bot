...
export function QuotesPage() {
  ...
  const selectedProduct=products.data?.find(p=>p.id===selectedProductId)||products.data?.[0];
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const customerId = String(data.get("customerId")); const expiresAt = String(data.get("expiresAt")); create.mutate({ ...(data.get("delivery")?{delivery:JSON.parse(String(data.get("delivery")))}:{}), productId: String(data.get("productId")), quantity: Number(data.get("quantity")), currency: String(data.get("currency")).toUpperCase(), customerId: customerId || null, expiresAt: expiresAt ? new Date(expiresAt).toISOString() : null }); }
